...
<div class="panel-body">
  ...
  <form name="mes_ano" method="get">
	...
	<h5><strong>Escolha quantos deseja exibir:</strong></h5>
      <div class="col-xs-2">
		  <select name="qt" class="form-control" id="sel1">
			<?php 
			  $m = 1;
			  while($m<=30){
     ...
     <div class="col-xs-3">
      <button type="submit" class="btn btn-default"><i class="fa fa-search" aria-hidden="true"></i> Confirmar</button>
	</div>
    </div>
  </form>
</div>
  ...
  <table class="table table-hover">
    ...
    <tbody>
     <?php 
			...
			$m = date("m");
			$a = date("Y");
			$cons = "SELECT * FROM checkout ORDER BY id DESC LIMIT $qt";
      ...
      <tr>
		...
		<?php }elseif($mostra["status"] == "Fechado"){ ?>
			<?php if($_SESSION["nivel"] == 1){ ?>
				<a href="caixa_edit.php?id=<?php echo $mostra["id"]; ?>" data-toggle="tooltip" title="Editar Caixa"><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a> - 
			<?php }else{} ?>
