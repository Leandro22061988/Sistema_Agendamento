	...
	if($_POST["pag"] == "n"){
		...
		if($_POST['id_prof']){
			foreach($ids as $valores){
				...
				if(isset($_POST["adicional"]) > 0){
					$adici = $_POST["adicional"];
				}else{
					$adici = 0.00;
				}
				if(isset($_POST["desconto"]) > 0){
					$desc = $_POST["desconto"];
				}else{
					$desc = 0.00;
				}
				...
				$sqlc = "UPDATE agenda SET status='$s', meio_pg='$p', parcelas='$parc', val_parcelas='$vparc', acrescimo='$adici', desconto='$desc' WHERE id_cliente='$c' and id_prof='$f' and status='$st' and dia='$d' and mes='$m' and ano='$a'";
				if($sql->query($sqlc) === true){
					...
					if($_POST["pag"] == "p"){
						...
						$sqlp = "INSERT INTO promissoria (codigo, cliente, id_cliente, id_prof, servico, valor, parcela, data_ini, vencimento) VALUES ('".$cod."', '".$cli."',  '".$c."', '".$f."', '".$serv."', '".$vparc."', '".$parc."', '".$paga."', '".$venc."')";
			...
			foreach($ids as $valores){
				...
				$sqlc = "UPDATE agenda SET status='$s', meio_pg='$p', parcelas='$parc', val_parcelas='$vparc', acrescimo='$adici', desconto='$desc' WHERE id_cliente='$c' and status='$st' and dia='$d' and mes='$m' and ano='$a'";
				if($sql->query($sqlc) === true){
					
					if($_POST["pag"] == "p"){
						$sqlig = "SELECT * FROM agenda WHERE id_cliente='$c' and dia='$d' and mes='$m' and ano='$a'";
						$execg = $sql->query($sqlig);
						$ag = mysqli_fetch_assoc($execg);
