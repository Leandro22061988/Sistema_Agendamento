<?php
	...
	$soma = 0;	
	$sqll = "SELECT * FROM adesao_curso WHERE parcelas >= 2";
	$exel = $sql->query($sqll);
	while($exibir = mysqli_fetch_assoc($exel)){
		
			$dias = $exibir["parcelas"];
			$valp = $exibir["val_parcelas"];
			$ini_d = $exibir["dia"]."-".$exibir["mes"]."-".$exibir["ano"];
			$ini_dir = $exibir["dia"]."/".$exibir["mes"]."/".$exibir["ano"];
